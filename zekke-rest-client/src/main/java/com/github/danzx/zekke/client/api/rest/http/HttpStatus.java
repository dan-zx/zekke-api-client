...
public enum HttpStatus {
    ...
    METHOD_FAILURE(420, "HttpMethod Failure"),
    ...
    REQUEST_HEADER_FIELDS_TOO_LARGE(431, "Request HttpHeader Fields Too Large"),
    ...
    public static HttpStatus fromCode(int statusCode) {
        ...
        for (HttpStatus current : values()) {
            if (current.code == statusCode) {
                status = current;
            }
        }
        if (status == null) throw new IllegalArgumentException("No matching constant for [" + statusCode + "]");
        return status;
    }
}
